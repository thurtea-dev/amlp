...
The entire C++20 LPC driver lives here. It compiles to the `lpcdriver` binary
...
Current baseline: **374 tests passing**. Every change must keep this green.
...
| `src/efun/` | EfunTable — all ~91 (→300+) registered efuns | [link](src/efun/instruct.md) |
...
See [`docs/ROADMAP.md`](../docs/ROADMAP.md) for the full phased plan with
per-task status checkboxes.

## Key invariants

- Namespace: everything lives in `namespace lpcdriver`.
