...
Full recognized set is in src/apply/ApplyTable.cpp. Currently fired: create(), init(), connect(), logon(), process_input(), net_dead(), compile_object(), heart_beat(), id(), catch_tell(). Recognized but not yet fired: clean_up, receive_message, valid_read, valid_write, valid_socket, get_root_uid, epilog, flag, short, long.
