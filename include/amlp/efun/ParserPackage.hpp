...
struct VerbRuleNode {
    ...
    // slice needs a per-*node* decision, not per-verb: a rule
    // containing any OBJ/LIV/OBS/LVS token is skipped entirely by
    // ParserPackage::parseSentence() (see its own comment) rather than
    // attempted and failed, since the noun-phrase-to-object resolution
    // engine those tokens need (ROADMAP.md row 0.13a's own item 8) is
    // not built yet -- a verb whose every registered rule needs an
    // object simply has no reachable interpretation yet, the same
    // honest "not yet supported, not silently wrong" stance already
    // used elsewhere in this row (m_values()'s own non-zero-column
    // rejection).
...
// confirmed against the real vendored header directly). Only None (no
// error, error_type left at the real "0" sentinel) and Allocated are
// reachable by anything this slice's own parseSentence() can produce --
// every other real kind (IsNot/NotLiving/NotAccessible/Ambig/Ordinal/
// ThereIsNo/BadMultiple/ManyPaths) is exclusively generated by
// object-token matching (parse_obj()/check_object_relations()/etc,
// packages/parser.c), none of which this slice's own matcher reaches --
// listed here anyway, with their real numeric values, so the noun-
// phrase-resolution slice extends this enum rather than renumbering it.
namespace ParserErrorType {
constexpr int None = 0;
constexpr int IsNot = 1;        // ERR_IS_NOT (not yet reachable)
constexpr int NotLiving = 2;    // ERR_NOT_LIVING (not yet reachable)
constexpr int NotAccessible = 3; // ERR_NOT_ACCESSIBLE (not yet reachable)
constexpr int Ambig = 4;        // ERR_AMBIG (not yet reachable)
constexpr int Ordinal = 5;      // ERR_ORDINAL (not yet reachable)
constexpr int Allocated = 6;    // ERR_ALLOCATED -- a can_/direct_/... callback's own explicit string return
constexpr int ThereIsNo = 7;    // ERR_THERE_IS_NO (not yet reachable)
constexpr int BadMultiple = 8;  // ERR_BAD_MULTIPLE (not yet reachable)
constexpr int ManyPaths = 9;    // ERR_MANY_PATHS (not yet reachable)
} // namespace ParserErrorType
...
public:
    ...
    // OBJ/LIV/OBS/LVS support yet (VerbRuleNode::hasObjectToken's own
    // comment explains why, and why this is a real, honest limitation
    // rather than a stub: a verb whose every rule needs an object
    // simply has no reachable interpretation yet). Real signature is
    ...
    // exclusively read by load_objects(), item 8, not reachable by
    // anything this slice's own matcher does). Returns int 1 on a
    // successful match (after invoking the winning do_* function, real
    // do_the_call()); otherwise a real, negative "how close did we get"
    // signal (`-foundLevel`, see .cpp) or, if some candidate's own
    // can_/direct_/... callback returned an explicit string, whatever
    // master()->parser_error_message() returns for it (real int 0 if
    // master does not define that apply, matching real code's own
    // "apply_master_ob() returned null" -> "*sp = const0" fallback).
