...
| 0.13 | Grow efun table to FluffOS parity (~300 efuns) | `src/efun` | [ ] (in progress: 240 registered; this exact vendored reference build's own efun_defs.c lists 270 real names total, not ~300 -- see STATUS.md's 2026-08-22/2026-08-23 entries. Real remaining gap (`comm -23`, not the raw registered-count difference -- see STATUS.md's own reconciliation) is 48: 40 non-`parse_*` names (documented exclusions/zero-call-site deferrals, re-verified 2026-08-24) plus the 8-name `parse_*` package itself, still the one deliberately-deferred dedicated-session candidate. `origin()` and `reload_object()` both implemented 2026-08-23; `functions`/`variables`/`fetch_variable`/`store_variable` implemented 2026-08-24, correcting an earlier "no implementation exists" miscall for all four (real bodies in `packages/contrib.c`); `socket_release`/`socket_acquire` implemented 2026-08-24 (continued), correcting an earlier "Tier 3, out of basics scope" miscall) |
