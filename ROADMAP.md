...
| # | Task | Directory | Status |
|---|------|-----------|--------|
| 1.1 | `LpcDialect` enum + config key `dialect` | `src/config` + `src/dialect` | [ ] (partial: the enum itself -- `LpcDialect`, `dialectName()`/`dialectFromString()` -- implemented 2026-08-24 in `src/dialect/LpcDialect.hpp`/`.cpp`, tested; no `Config::dialect()` config key wired up yet, so nothing in the driver actually reads or acts on it) |
...
| 1.4 | Pluggable boot API: FluffOS master/simul_efun, LDMud master, DGD driver+auto | `src/apply` + `src/dialect` | [ ] (partial: `BootApi` abstract base plus `FluffOsBootApi`/`LdmudBootApi` implemented 2026-08-24 in `src/dialect`, deliberately trimmed to omit `connectApply()`/`netDeadApply()` and `DgdBootApi` per the still-open design note on this row -- see `src/dialect/instruct.md`. No `DialectFactory` yet, and `ApplyTable`/`VM::applyMaster()` call sites still hardcode apply name strings directly (`Server.cpp`, `ObjectManager.cpp`) rather than routing through `BootApi` -- the actual pluggable dispatch this row is asking for is not wired up yet) |
...
| 1.16 | LDMud master apply name table | `src/apply` + `src/dialect` | [ ] (partial: only the corrected `get_master_uid` name -- not FluffOS's `get_root_uid`, superseded in LDMud 3.2.1@40 -- implemented and tested 2026-08-24 via `LdmudBootApi::masterUidApply()`. The rest of this row's own recorded apply surface -- `get_bb_uid`, `valid_read`/`valid_write`, `make_path_absolute`, `query_allow_shadow`, `valid_snoop`/`valid_query_snoop` -- still not implemented) |
