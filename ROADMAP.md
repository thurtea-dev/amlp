...
**Next slice recommendation:** with (1) through (5) all landed, (6) the sentence tokenizer and (8) the noun-phrase resolution engine are both real prerequisites for anything resembling actual `parse_sentence()` behavior and should likely be tackled together (or at minimum in that order) rather than separately, since (9)'s matcher needs both before it can do anything observable. (7) (the fuller `parse_info_t`/`parse_refresh()`/`parse_free()`) is a smaller, more contained follow-on that could reasonably slot in either just before or just after (6)/(8), since `parse_refresh()` specifically exists to invalidate the very caches (8) introduces.) |
