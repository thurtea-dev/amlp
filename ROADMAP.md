...
| 0.13a | `parse_*` natural-language sentence/grammar-rule parser package (`parse_init`, `parse_refresh`, `parse_sentence`, `parse_add_rule`, `parse_add_synonym`, `parse_my_rules`, `parse_dump`, `parse_remove`) | `src/efun` | [ ] (**greenlit 2026-08-18 as a real multi-session project; first real slice landed this session** -- `parse_init()`/`parse_add_rule()`/`parse_dump()`/`parse_remove()` implemented, all four backed by a real, faithfully-ported rule-string tokenizer and a real verb/rule registry. `parse_refresh`/`parse_sentence`/`parse_add_rule`'s 3-arg shadow form/`parse_add_synonym`/`parse_my_rules` remain unimplemented -- see the full component breakdown and next-slice ranking below, written so the next session does not need to re-read `packages/parser.c` (3419 lines) from scratch.

**Full real component breakdown** (confirmed by reading the entire real source, `packages/parser.c` + `packages/parser.h`, not assumed from the file's size alone):
...
**Next slice recommendation:** with (1) through (5) and now (7)'s separable half both landed, (6) the sentence tokenizer and (8) the noun-phrase resolution engine (which would also finally make interrogate_object()/the rest of (7)'s cache real) are the two remaining real prerequisites for anything resembling actual `parse_sentence()` behavior, and should be tackled together, since (9)'s matcher needs both before it can do anything observable and neither is separately live-testable on its own (confirmed this session for (6) too: its own word-splitting front end has no real efun surface of its own to expose it through independent of the matcher it feeds).) |
...
Current baseline: **634 tests passing** (as of 2026-08-18). Every new slice
