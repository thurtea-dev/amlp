...
| 0.13 | Grow efun table to FluffOS parity (~300 efuns) | `src/efun` | [ ] (in progress: 238 registered; this exact vendored reference build's own efun_defs.c lists 270 real names total, not ~300 -- see STATUS.md's 2026-08-22/2026-08-23 entries. Real remaining gap (`comm -23`, not the raw registered-count difference -- see STATUS.md's own reconciliation) is 50. `origin()` and `reload_object()` both implemented 2026-08-23; `functions`/`variables`/`fetch_variable`/`store_variable` implemented 2026-08-24, correcting an earlier "no implementation exists" miscall for all four (real bodies in `packages/contrib.c`, confirmed live this session). Remaining gap is entirely prior-documented exclusions/deferrals plus one dedicated-session candidate left -- the real parser package, `packages/parser.c`) |
