...
| 1.16 | LDMud master apply name table | `src/apply` + `src/dialect` | [ ] (partial: only the corrected `get_master_uid` name -- not FluffOS's `get_root_uid`, superseded in LDMud 3.2.1@40 -- implemented and tested 2026-08-24 via `LdmudBootApi::masterUidApply()`, and wired into a real runtime call path 2026-08-24 (continued) via `queryMasterUid()` (see row 1.4). The rest of this row's own recorded apply surface -- `get_bb_uid`, `valid_read`/`valid_write`, `make_path_absolute`, `query_allow_shadow`, `valid_snoop`/`valid_query_snoop` -- still not implemented) |
