...
| 0.13 | Grow efun table to FluffOS parity (~300 efuns) | `src/efun` | [ ] (in progress: 233 registered; this exact vendored reference build's own efun_defs.c lists 270 real names total, not ~300 -- see STATUS.md's 2026-08-22/2026-08-23 entries. `origin()` implemented 2026-08-23, full per-call-path citation trail in VM.hpp/VM.cpp. Remaining 37-name gap is now entirely prior-documented exclusions/deferrals: shadowed simul_efuns, TYPE_BUFFER/TYPE_CLASS architecture mismatches, unverifiable no-body efuns, and two dedicated-session candidates -- the real parser package, reload_object()) |
