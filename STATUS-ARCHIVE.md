...
**Eighth archival pass, 2026-08-21 (later still the same day):** moved
the single oldest entry (the "privilege_violation()'s real cold-start
scoping investigation" session, bind_lambda()/set_driver_hook()) into
this file, verbatim, no content altered, to keep `STATUS.md` to its own
5-most-recent-sessions rule after this session's new entry (the
save_object()/restore_object() width > 1 mapping stopgap fix, plus
login integration's build ordering item 4, character creation, of
`notes/ACCOUNT_LOGIN_PLAN.md`). Same discipline, same insertion
placement as the seven passes above.
